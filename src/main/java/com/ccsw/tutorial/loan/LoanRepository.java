...
public interface LoanRepository extends CrudRepository<Loan, Long>, JpaSpecificationExecutor<Loan> {
    @Query("SELECT CASE WHEN COUNT(l) > 0 THEN true ELSE false END FROM Loan l " + "WHERE l.game.id = :gameId " + "AND l.loanDate <= :returnDate " + "AND l.returnDate >= :loanDate " + "AND (:loanId IS NULL OR l.id <> :loanId)")
